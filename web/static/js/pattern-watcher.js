...
class PatternWatcher {
    ...
    setupEventListeners() {
        ...
        // Control buttons
        document.getElementById('refresh-symbols-btn').addEventListener('click', () => {
            this.loadSymbols();
        });

        document.getElementById('scan-patterns-btn').addEventListener('click', () => {
    ...
    async loadSymbols() {
        ...
        try {
            ...
            console.log(`Loaded ${data && data.symbols ? data.symbols.length : 0} symbols`);
    ...
    displaySymbols() {
        ...
        container.innerHTML = filteredSymbols.map(symbol => 
            this.createSymbolListItem(symbol)
        ).join('');
    ...
    createSymbolListItem(symbolData) {
        ...
        const symbol = typeof symbolData === 'string' ? symbolData : symbolData.symbol || symbolData.Symbol;
        const symbolName = typeof symbolData === 'object' ? (symbolData.name || symbolData.Name || '') : '';
            ...
            <div class="symbol-item" data-symbol="${symbol}">
                <div class="d-flex justify-content-between align-items-start">
                    <div class="flex-grow-1">
                        ...
                        ${symbolName ? `<div class="text-muted small">${symbolName}</div>` : ''}
                        ${patternBadges}
                        <div class="symbol-stats">
                            <div class="stat-item">
                                <div class="stat-value">${symbolPatterns.length}</div>
                                <div>Patterns</div>
                            </div>
                            <div class="stat-item">
                                <div class="stat-value">${symbolPatterns.filter(p => !p.is_complete).length}</div>
                                <div>Active</div>
                            </div>
                            <div class="stat-item">
                                <div class="stat-value">${this.getAvgCompletion(symbolPatterns)}%</div>
                                <div>Avg Progress</div>
                            </div>
                        </div>
                    </div>
                    <div class="symbol-status ${this.getSymbolStatus(symbolPatterns)}"></div>
                </div>
            </div>
        `;
    }
    ...
    createPatternBadges(patterns) {
        ...
        const badges = uniqueTypes.map(type => 
            `<span class="pattern-badge ${type.replace(/_/g, '-')}">${this.formatPatternType(type)}</span>`
        ).join('');
    ...
    displayNoPatterns() {
        ...
        container.innerHTML = `
            <div class="text-center p-4 text-muted">
                ...
                <p class="mt-3">No patterns detected</p>
                <small>Select a symbol and click "Detect Patterns"</small>
            </div>
        `;
    }
                ...
                <div class="d-flex justify-content-between align-items-start">
                    <div class="flex-grow-1">
                        <div class="d-flex align-items-center mb-2">
                            <h6 class="mb-0 pattern-type pattern-type-${pattern.pattern_type.replace(/_/g, '-')}">${pattern.symbol}</h6>
                            <span class="badge pattern-phase ${phaseClass} ms-2">
                                <i class="bi bi-${phaseIcon} me-1"></i>${this.formatPhase(pattern.current_phase)}
                            </span>
                        </div>
                        <div class="small text-muted mb-1">${this.formatPatternType(pattern.pattern_type)}</div>
                        <div class="d-flex justify-content-between text-sm mb-1">
                            <span class="text-muted">Progress:</span>
                            <span class="fw-bold text-pw-primary">
                                ${pattern.thesis_components?.completed_components || 0}/${pattern.thesis_components?.total_components || 0}
                            </span>
                        </div>
    ...
    async selectSymbol(symbol) {
        ...
        try {
            ...
            document.getElementById('pattern-info').textContent = `Ready to analyze patterns for ${symbol}`;
    ...
    loadThesisComponents(pattern) {
        ...
        document.getElementById('thesis-symbol').textContent = `- ${pattern.symbol} (${this.formatPatternType(pattern.pattern_type)})`;
        document.getElementById('thesis-progress').style.width = `${thesis.completion_percent || 0}%`;
        document.getElementById('thesis-completion').textContent = `${thesis.completed_components || 0}/${thesis.total_components || 0}`;
            ...
            <div class="thesis-section">
                ...
                <div class="mb-3">
                    <div class="d-flex justify-content-between">
                        ...
                        <span class="text-pw-primary">${this.formatPatternType(pattern.pattern_type)}</span>
                    </div>
                    <div class="d-flex justify-content-between">
                        <span>Current Phase:</span>
                        <span class="text-pw-primary">${this.formatPhase(pattern.current_phase)}</span>
                    </div>
                    <div class="d-flex justify-content-between">
                        <span>Completion:</span>
                        <span class="text-pw-primary">${thesis.completion_percent || 0}%</span>
                    </div>
                </div>
                <div class="pattern-progress">
                    <div class="pattern-progress-fill" style="width: ${thesis.completion_percent || 0}%"></div>
                </div>
    ...
    getFilteredSymbols() {
        ...
        return this.symbols.filter(symbolData => {
            // Handle both string symbols and symbol objects
            const symbol = typeof symbolData === 'string' ? symbolData : symbolData.symbol || symbolData.Symbol;
            return symbol && symbol.toLowerCase().includes(searchTerm);
        });
    }
    ...
    getFilteredPatterns() {
        ...
        if (this.currentFilter !== 'all') {
            filtered = filtered.filter(pattern => pattern.pattern_type === this.currentFilter);
        }
    ...
    getAvgCompletion(patterns) {
        ...
        const total = patterns.reduce((sum, p) => sum + (p.thesis_components?.completion_percent || 0), 0);
    ...
    // Action methods
    showAddSymbolModal() {
        const modal = new bootstrap.Modal(document.getElementById('add-symbol-modal'));
    ...
    async quickAddSymbol() {
        ...
        try {
            ...
            if (response.ok) {
                input.value = '';
                this.showSuccess(`Symbol ${symbol} added to watchlist. Automatic pattern detection started.`);
                await this.loadSymbols();
    ...
    async scanAllPatterns() {
        try {
            ...
            if (response.ok) {
                ...
                this.showSuccess(`Pattern scan completed. ${result.patterns_updated || 'Multiple'} patterns updated.`);
    ...
    async detectPatternsForSelected() {
        ...
        try {
            ...
            const response = await fetch(`/api/head-shoulders/symbols/${this.selectedSymbol}/detect`, {
                method: 'POST'
            });
            ...
            if (response.ok) {
                ...
                if (result.status === 'pattern_detected') {
                    ...
                    this.showInfo(`No patterns found for ${this.selectedSymbol}. ${result.message}`);
                } else {
                    this.showSuccess(`Pattern detection completed for ${this.selectedSymbol}`);
                    await this.loadPatterns();
                }
            } else {
    ...
    updatePatternCount() {
        document.getElementById('pattern-count').textContent = this.getFilteredPatterns().length;
    }
    ...
    async saveComponentUpdate() {
        ...
        const evidence = document.getElementById('update-evidence').value.split('\n').filter(line => line.trim());
        ...
        try {
            const response = await fetch(`/api/head-shoulders/pattern/${patternId}/thesis/${componentName}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
