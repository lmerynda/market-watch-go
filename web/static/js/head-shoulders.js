...
class HeadShouldersMonitor {
  ...
  setupEventListeners() {
    ...
    // Control buttons
    document
      .getElementById("refresh-patterns-btn")
      .addEventListener("click", () => {
        this.loadPatterns();
      });

    document
      .getElementById("monitor-patterns-btn")
      .addEventListener("click", () => {
    ...
    document
      .getElementById("toggle-thesis-overlay")
      .addEventListener("click", () => {
  ...
  async loadPatterns() {
    ...
    try {
      ...
      const response = await fetch(
        "/api/head-shoulders/patterns?pattern_type=inverse_head_shoulders"
      );
  ...
  displayPatterns() {
    ...
    container.innerHTML = filteredPatterns
      .map((pattern) => this.createPatternListItem(pattern))
      .join("");
                ...
                <div class="d-flex justify-content-between align-items-start">
                    <div class="flex-grow-1">
                        <div class="d-flex align-items-center mb-2">
                            <h6 class="mb-0 pattern-symbol text-primary">${
                              pattern.symbol
                            }</h6>
      ...
      pattern.current_phase
                        ...
                        <div class="d-flex justify-content-between text-sm mb-1">
                            ...
                            <span class="fw-bold text-tv-primary">
                                ${
                                  pattern.thesis_components.completed_components
                                }/${pattern.thesis_components.total_components}
                            </span>
                        </div>
                ...
                <div class="mt-2 pt-2 border-top border-tv">
                    <small class="text-tv-secondary">
                        Target: <span class="text-success">$${this.calculateTarget(
                          pattern
                        ).toFixed(2)}</span> |
                        Symmetry: <span class="text-info">${pattern.symmetry.toFixed(
                          1
                        )}%</span>
                    </small>
                </div>
  ...
  getPatternIndicators(pattern) {
    ...
    if (pattern.thesis_components.breakout_volume?.is_completed) {
      indicators.push(
        '<i class="bi bi-bar-chart-fill text-volume" title="Volume Confirmed"></i>'
      );
    } else {
      indicators.push(
        '<i class="bi bi-bar-chart text-muted" title="Volume Pending"></i>'
      );
    }
    ...
    if (pattern.thesis_components.neckline_breakout?.is_completed) {
      indicators.push(
        '<i class="bi bi-arrow-up-circle-fill text-breakout" title="Breakout Confirmed"></i>'
      );
    } else {
      indicators.push(
        '<i class="bi bi-arrow-up-circle text-muted" title="Breakout Pending"></i>'
      );
    }
    ...
    if (pattern.thesis_components.full_target?.is_completed) {
      indicators.push(
        '<i class="bi bi-bullseye text-target" title="Target Reached"></i>'
      );
    } else if (pattern.thesis_components.partial_fill_t1?.is_completed) {
      indicators.push(
        '<i class="bi bi-record-circle text-target" title="Partial Target"></i>'
      );
    } else {
      indicators.push(
        '<i class="bi bi-circle text-muted" title="Target Pending"></i>'
      );
    }
  ...
  getFilteredPatterns() {
    ...
    if (this.currentFilter !== "all") {
      filtered = filtered.filter(
        (pattern) => pattern.current_phase === this.currentFilter
      );
    }

    // Apply search filter
    const searchTerm = document
      .getElementById("pattern-search")
      .value.toLowerCase();
    if (searchTerm) {
  ...
  async selectPattern(patternId) {
    ...
    try {
      ...
      document.getElementById(
        "pattern-status"
      ).textContent = `${this.formatPhase(pattern.current_phase)} - ${
        pattern.thesis_components.completed_components
      }/${pattern.thesis_components.total_components} components`;
  ...
  loadThesisComponents(pattern) {
    ...
    document.getElementById(
      "thesis-symbol"
    ).textContent = `- ${pattern.symbol}`;
    document.getElementById(
      "thesis-progress"
    ).style.width = `${thesis.completion_percent}%`;
    document.getElementById(
      "thesis-completion"
    ).textContent = `${thesis.completed_components}/${thesis.total_components}`;
    ...
    const sections = [
      ...
      },
      {
        ...
        components: [thesis.neckline_breakout, thesis.neckline_retest],
      },
      {
    ...
    container.innerHTML = sections
      .map((section) => this.createThesisSection(section, pattern.id))
      .join("");
  }

  createThesisSection(section, patternId) {
    const completedCount = section.components.filter(
      (c) => c && c.is_completed
    ).length;
                    ...
                    ${section.components
                      .map((component) =>
                        component && component.name
                          ? this.createThesisComponent(component, patternId)
                          : ""
                      )
                      .join("")}
                </div>
  ...
  createThesisComponent(component, patternId) {
    ...
    const statusIcon = component.is_completed
      ? "check-circle-fill"
      : component.is_required
      ? "exclamation-circle"
      : "circle";
    const statusClass = component.is_completed
      ? "text-success"
      : component.is_required
      ? "text-warning"
      : "text-muted";
    ...
    return `
            <div class="col-md-6 mb-2">
                <div class="thesis-component ${
                  component.is_completed ? "completed" : ""
                } ${component.is_required ? "required" : ""}"
                     onclick="window.hsMonitor.openComponentModal('${
                       component.name
                     }', ${patternId})">
                    <div class="component-status">
                        ...
                        ${
                          completedAt
                            ? `<small class="text-muted">${completedAt}</small>`
                            : ""
                        }
                    </div>
                    ${
                      component.description
                        ? `<div class="component-evidence">${component.description}</div>`
                        : ""
                    }
                    ${
                      component.evidence && component.evidence.length > 0
                        ? `<div class="component-evidence">• ${component.evidence.join(
                            "<br>• "
                          )}</div>`
                        : ""
                    }
                    ${
                      component.confidence_level > 0
                        ? `<div class="component-confidence">Confidence: ${component.confidence_level.toFixed(
                            0
                          )}%</div>`
                        : ""
                    }
  ...
  openComponentModal(componentName, patternId) {
    ...
    document.getElementById("update-is-completed").checked =
      component.is_completed;
    document.getElementById("update-confidence").value =
      component.confidence_level || 50;
    document.getElementById("confidence-display").textContent =
      (component.confidence_level || 50) + "%";
    document.getElementById("update-evidence").value = component.evidence
  ...
  async saveComponentUpdate() {
    ...
    try {
      const response = await fetch(
        `/api/head-shoulders/pattern/${patternId}/thesis/${encodeURIComponent(
          componentName
        )}`,
        {
          method: "PUT",
  ...
  async monitorAllPatterns() {
    try {
      ...
      this.showSuccess(
        `Monitoring completed. ${result.patterns_updated} patterns updated.`
      );
  ...
  async detectPatternForSelected() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/head-shoulders/symbols/${this.selectedPattern.symbol}/detect`,
        {
          method: "POST",
        }
      );
      ...
      this.showSuccess(
        `Pattern detection completed for ${this.selectedPattern.symbol}`
      );
